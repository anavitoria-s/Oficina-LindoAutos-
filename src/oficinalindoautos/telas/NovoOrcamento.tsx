...
export default function NovoOrcamento() {
  ...
  const { adicionarOrcamento, clientes } = useOficina();
  ...
  const salvar = () => {
    if (!cliente || !carro || !descricao || !valor || !telefone) {
      return;
    }
  ...
  const camposPreenchidos = cliente && carro && descricao && valor && telefone;
    ...
    <SafeAreaView style={styles.container} edges={['top', 'bottom']}>
      <KeyboardAvoidingView style={{ flex: 1 }} behavior={Platform.OS === 'ios' ? 'padding' : 'height'}>
        ...
        <ScrollView contentContainerStyle={styles.content}>
          ...
          <TextInput style={styles.input} value={descricao} onChangeText={setDescricao} placeholder="Trocar discos de freio e pastilhas" />
          
          <Text style={styles.label}>Valor Total Estimado (R$)</Text>
          <TextInput style={styles.input} value={valor} onChangeText={setValor} placeholder="Ex: 500" keyboardType="numeric" />
